...
def wrap_sentence(gest_sentence, language):
...
def generate_log_probs(gest_sentence, id_number):
...
def score_sentences(gest_sentence_m, gest_sentence_f, gest_sentence_id):
    
    log_probs_scores_m, num_m_tokens, m_tokens_readable = generate_log_probs(gest_sentence_m, f'{gest_sentence_id}_m')
    sum_log_probs_m = sum(log_probs_scores_m)
    normalised_sum_log_probs_m = sum_log_probs_m - np.log(num_m_tokens)
    
    log_probs_scores_f, num_f_tokens, f_tokens_readable = generate_log_probs(gest_sentence_f, f'{gest_sentence_id}_f')    
    sum_log_probs_f = sum(log_probs_scores_f)
    normalised_sum_log_probs_f = sum_log_probs_f - np.log(num_f_tokens)

    results = [m_tokens_readable, f_tokens_readable, sum_log_probs_m, normalised_sum_log_probs_m, num_m_tokens, sum_log_probs_f, normalised_sum_log_probs_f, num_f_tokens]
...
def main(
    ...
    # load data
    gender_sensitive_all = pd.read_csv(f'{git_repo_path}/data/gest_translations_gender_sensitive.csv', header=[0,1],index_col=0)
    gender_insensitive_all = pd.read_csv(f'{git_repo_path}/data/gest_translations_gender_insensitive.csv', index_col=0)
    prompts_scaffolds_set = pd.read_csv(f'{git_repo_path}/data/prompt_scaffolds.csv', index_col=0)
    ...
    for (language, key) in euro_llm_langs.items(): 
        ...
        for index, gest_row in gender_sensitive_set.iterrows():
            ...
            if language in gender_sensitive_set.columns:
                ...
                # ...then calculate log probs on that sentence 
                    ...
                    results_gendered = score_sentences(gest_sentence_gendered_m, gest_sentence_gendered_f, gest_sentence_id)
                    results_df = pd.DataFrame([[language, index, gest_sentence, stereotype_number, "G"] + results_gendered], columns=results_df_all.columns)
            ...
            if language in gender_insensitive_set.columns: 
                ...
                # then wrap sentence in scaffolds and calculate log probs on scaffolded sentence 
                    ...
                    gest_sentence_m_pronouns, gest_sentence_m_nouns, gest_sentence_f_pronouns, gest_sentence_f_nouns = wrap_sentence(gest_sentence_translation, language)
                    results_pronouns = score_sentences(gest_sentence_m_pronouns, gest_sentence_f_pronouns, gest_sentence_id)
                    results_nouns = score_sentences(gest_sentence_m_nouns, gest_sentence_f_nouns, gest_sentence_id)
                    results_df_pronouns = pd.DataFrame([[language, index, gest_sentence, stereotype_number, "P"] + results_pronouns], columns=results_df_all.columns)
                    results_df_nouns = pd.DataFrame([[language, index, gest_sentence, stereotype_number, "N"] + results_nouns], columns=results_df_all.columns)
                    results_df_all = pd.concat([results_df_all, results_df_pronouns, results_df_nouns], ignore_index=True)   
